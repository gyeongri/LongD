...
public class GalleryController {
    ...
    public ResponseEntity<?> getGalleryList(@PathVariable int coupleListId, @RequestParam(required = false) String folderName ) {
        try {
            List<Gallery> list = null;
            if( folderName == null ) {
                list = galleryService.getGalleryList(coupleListId);
            } else {
                list = galleryService.getGalleryFolderList(coupleListId, folderName);
            }
